...
- **Per-Lesson Diagram Generation**: Diagrams are no longer a separate whole-unit pass that ran only after every lesson's text was finished. `generateUnitContent` now renders each lesson's diagram(s) immediately after that lesson's text and streams the updated lesson again, so visuals pop in lesson-by-lesson while later lessons are still being written (`_attachArtToLesson` holds the per-lesson art logic; the old `generateCanvasArtForUnit` second pass was removed).
