...
- **Module Quick Review**: Introduced a per-module "Quick Review" feature accessible directly from the dashboard toolbar via a new sparkles button. It generates key theory statements and formulas (including LaTeX equations, omitting derivations and proofs) using section PDF content or fallback TOC skeletons.
- **AI-Powered Concept Expansion & Navigation**: Clicking any quick review statement opens a detailed explanation bottom sheet generated dynamically by the AI. Additionally, it matches the statement's topic against the module's generated lessons and offers a "Study Related Lesson" button to jump directly into the interactive lesson player.
