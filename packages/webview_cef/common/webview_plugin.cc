...
namespace webview_cef {
	...
	void initCEFProcesses(){
		...
		// exitCode >= 0 means this IS a subprocess (renderer, GPU, etc.).
		// Exit immediately so the subprocess never falls through into the
		// host app's GTK/Flutter startup code, which would otherwise trigger
		// ATK socket-embed assertions and cursor-theme warnings.
		if (exitCode >= 0) {
			exit(exitCode);
		}
	}
	...
	void startCEF()
	{
		...
		// Set app-specific cache path to avoid singleton conflicts between apps.
		{
			const char* cacheBase = getenv("XDG_CACHE_HOME");
			std::string cachePath;
			if (cacheBase && *cacheBase) {
				cachePath = std::string(cacheBase) + "/duofy";
			} else {
				const char* home = getenv("HOME");
				if (home && *home) {
					cachePath = std::string(home) + "/.cache/duofy";
				}
			}
			if (!cachePath.empty()) {
