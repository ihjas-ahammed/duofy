...
if (-not $GRAPHIFY_PYTHON) {
    Write-Host "Graphify not found in python. Installing..."
    if (Get-Command uv -ErrorAction SilentlyContinue) {
...
if ($GRAPHIFY_PYTHON) {
    ...
    Write-Host "Graphify Python path successfully saved to graphify-out\.graphify_python: $GRAPHIFY_PYTHON"
} else {
    Write-Error "Failed to install or locate graphify."
    exit 1
}
